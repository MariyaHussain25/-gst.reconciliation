...
class ReconciliationResult(BaseModel):
    ...
    gstr2a_invoice_number: Optional[str] = None
    ...
    portal_available_itc: float = 0.0
    # ── AI explanation ────────────────────────────────────────────────────────
...
class ReconciliationSummary(BaseModel):
    ...
    # Invoices in GSTR-2A but NOT in GSTR-2B — ITC cannot be claimed
    missing_in_2b_count: int = 0
    # Invoices in GSTR-2B but NOT in GSTR-2A — supplier filed but company hasn't recorded
